<?php
	...
	if($_POST["add"] == "Add") {
		...
		$obj = new DetailPermohonanDanaModel();
		$obj->id_permohonan_dana = $_POST["id_pengeluaran"];
		...
		$obj->pph = $pph;
		$obj->Insert();
		header("location:" . $_SERVER["PHP_SELF"] . "?id=" . $_POST["id_pengeluaran"] . "&pd=" . $_POST["pd"]);
	}
	...
	if($_POST["add_normatif"] == "Add") {
		...
		";
		$db->add_parameter("i", $_POST["id_pengeluaran"]);
	...
	if($_GET["delete"] == 1) {
		$obj = new DetailPermohonanDanaModel();
		$obj->Delete($_GET["id"]);
		header("location:" . $_SERVER["PHP_SELF"] . "?id=" . $_GET["id_pengeluaran"] . "?pd=" . $_GET["pd"]);
	}
	...
	if($_POST["save"] == "Save") {
		...
		$obj->Update($_POST["id_pengeluaran"]);
		header("location:cetak_berkas_host.php?id=" . $_POST["id_pengeluaran"] . "&pd=" . $_POST["pd"]);
	}
	...
	$data_detail = DetailPermohonanDanaBusinessLogic::GetList02($_GET["id"]);
