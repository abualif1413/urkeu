<?php
	...
	if($_POST["save"] == "Save") {
		$db = new DBConnection();
		$db->perintahSQL = "
			INSERT INTO itbl_apps_penerimaan_lain (
				tanggal, keterangan, 
				jumlah, user_insert
			) VALUES(
				?, ?, 
		...
		$db = new DBConnection();
		$db->perintahSQL = "
			UPDATE itbl_apps_penerimaan_lain SET
				tanggal=?, keterangan=?, 
				jumlah=?, user_insert=?
			WHERE
				id=?
	...
	if($_GET["hapus"] == 1) {
		$db = new DBConnection();
		$db->perintahSQL = "DELETE FROM itbl_apps_penerimaan_lain WHERE id = ?";
		$db->add_parameter("i", $_GET["id"]);
		$db->execute_non_query();
	...
	echo $twig->render('penerimaan_lain.php', array(
